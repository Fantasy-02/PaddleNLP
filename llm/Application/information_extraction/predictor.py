...
class PredictorArgument:
    ...
    max_length: int = field(default=200, metadata={"help": "the max length for decoding."})
...
def compute_metrics(data_file):
    ...
    with open(data_file, "r", encoding = "utf-8") as f:
        for idx, line in enumerate(f, 1):
            ...
            ind = data["tgt"].find("\n**回答结束**\n\n")
            if ind != -1:
                label = set(string_to_list(data["tgt"][:ind]))
            else:
                label = set(string_to_list(data["tgt"]))

            ind1 = data["output"].find("\n**回答结束**\n\n")
            if ind1 != -1:
                pred = set(string_to_list(data["output"][:ind1]))
            else:
                pred = set(string_to_list(data["output"]))
